...
from krkn_ai.models.config import HealthCheckResult, OutputConfig
...
class TestHealthCheckReporter:
    ...
    def test_save_report_with_health_check_results(self, temp_output_dir):
        ...
        report_path = os.path.join(temp_output_dir, "reports", "health_check_report.csv")
    ...
    def test_sort_fitness_result_csv_sorts_by_fitness_score(self, temp_output_dir):
        ...
        results = [
            CommandRunResult(
                ...
                start_time=now,
                end_time=now,
                fitness_result=FitnessResult(fitness_score=float(10 - i * 2))  # 10, 8, 6, 4, 2
            )
            for i in range(5)
        ]
        ...
        assert all(df.iloc[i]['fitness_score'] >= df.iloc[i+1]['fitness_score'] 
