...
def _safe_uid(uid: str) -> str:
    if not uid or len(uid) > 253 or any(c not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789._-" for c in uid):
...
def _safe_path(path: str) -> PurePosixPath:
    ...
    if not path or "\\" in path or candidate.is_absolute() or any(part in ("", ".", "..") for part in candidate.parts):
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "invalid artifact path")
    return candidate
...
class ArtifactStore:
    ...
    def commit(self, uid: str, request: CommitRequest) -> dict[str, Any]:
        ...
        for entry in request.files:
            ...
            if normalized in seen:
                raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "duplicate manifest path")
            seen.add(normalized)
            file_path = run_dir / relative_path
            if not file_path.is_file():
                raise HTTPException(status.HTTP_409_CONFLICT, f"artifact {normalized} has not been uploaded")
            actual_sha256, actual_size = _sha256(file_path)
            if actual_size != entry.size or not hmac.compare_digest(actual_sha256, entry.sha256):
                raise HTTPException(status.HTTP_409_CONFLICT, f"artifact {normalized} does not match manifest")
            files.append({"path": normalized, "sha256": entry.sha256, "size": entry.size})
        ...
        if manifest_path.exists():
            if manifest_path.read_bytes() == encoded:
                return manifest
            raise HTTPException(status.HTTP_409_CONFLICT, "run artifacts are already committed")
        descriptor, temporary_name = tempfile.mkstemp(prefix=".manifest-", dir=manifest_path.parent)
        try:
            with os.fdopen(descriptor, "wb") as temporary:
    ...
    def manifest(self, uid: str) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status.HTTP_404_NOT_FOUND, "run artifacts are not committed") from exc
...
def create_app(root: str | Path | None = None, token: str | None = None) -> FastAPI:
    store = ArtifactStore(root or os.environ.get("KRKNAI_ARTIFACT_ROOT", DEFAULT_ARTIFACT_ROOT))
    service_token = token if token is not None else os.environ.get("KRKNAI_SERVICE_TOKEN", "")
    app = FastAPI(title="Krkn-AI artifact service")

    def authenticate(authorization: Annotated[str | None, Header()] = None) -> None:
        expected = f"Bearer {service_token}"
        if not service_token or authorization is None or not hmac.compare_digest(authorization, expected):
    ...
    def discover(request: DiscoveryRequest) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "invalid kubeconfig encoding") from exc
        ...
        try:
            ...
            try:
                ...
                warnings = ["Prometheus discovery failed; generated configuration omits Prometheus data."]
    ...
    @app.put(
        "/v1/runs/{uid}/files/{path:path}", dependencies=[Depends(authenticate)]
    )
    async def upload(
        uid: str,
        path: str,
        ...
        size = await store.upload(
            uid, path, request.stream(), x_checksum_sha256 or ""
        )
...
def upload_results() -> None:
    ...
    state = _load_state()
    files: list[dict[str, Any]] = []
    for source in sorted(path for path in output.rglob("*") if path.is_file() and path != marker):
