...
def get_pvc_usage_percentage(
    ...
    try:
        ...
        if not pod_name or not volume_name:
            logger.debug("No running pod found using PVC %s in namespace %s", pvc_name, namespace)
            return None
        ...
        if len(command_output) < 4:
            logger.debug("Unexpected df output format for PVC %s: %s", pvc_name, command_output)
            return None
        ...
        if pvc_capacity_kb > 0:
            ...
            if cache_key not in _logged_pvcs:
                logger.info("Found PVC %s usage: %.2f%% (used: %d KB, capacity: %d KB)", 
